<?php
    ...
    if(!isset($_SESSION['USER_ID']) || $loggdUType != "Admin" && $loggdUType != "Admin Telemarketing" && $loggdUType != "Telemarketing" && $loggdUType != "Business Unit Manager")
     {
     ...
     if(isset($_POST['submitcheck'])){
       ...
       $arrVal = array(
         ...
         $name_campaign = $getrefid['name'];
         $cantidad_verificada = $getrefid['verificado'];

         $arrVal = array(
                         "verificado" => $cantidad_verificada+1
                        );
    ...
    if(isset($_POST['submitSuspect']))
     {
          ...
          if($view==0){
            ...
            UpdateRec("suspects", "id = ".$_GET['id_suspect'], $updateView);

            $updateCampaign = array(
                          "sospechoso" => $sospechoso+1
                         );
            UpdateRec("campaign", "id = ".$_GET['id_campaign'], $updateCampaign);
          ...
          <section class="scrollable padder">
              	...
              	<form class="form-horizontal" action="" data-validate="parsley" method="post" enctype="multipart/form-data" novalidate>
                    ...
                    <section class="panel panel-default">
                      <header class="panel-heading">
                        <span class="h4">Ver Campaña: <?php echo $arrUser[0]['name']; ?> <a href="modal_detail_oportunity.php?id_campaign=<?php echo $_GET['id_campaign']?>" data-toggle="ajaxModal" class="btn btn-sm btn-default">Ver más</a></span>
                      </header>
                      <div class="panel-body">
                        <span class="h4">Sospechoso </span>
                        ...
                        <div class="form-group">
                          ...
                          <div class="col-lg-2">
                            ...
                            <?php if($contar >= 1){ ?>
                            <select class="chosen-select form-control" name="business" required="required" onChange="ocultar();">
                              <option value="">--------------</option>
                              <?PHP
                          ...
                          <?php }else{ ?>
                            <input type="text" class="form-control" placeholder="Empresa" name="business" data-required="true" value="<?php if($name_business != ""){
                          ...
                          <div class="col-lg-2">
                            <input type="text" class="form-control" placeholder="Contacto" name="contact" value="<?php if($contacto != ""){ echo $contacto; } ?>">
                          </div>
                          <div class="col-lg-1">
                          </div>
                        ...
                        <div class="form-group">
                          ...
                          <div class="col-lg-2">
                            <select class="chosen-select form-control" name="item">
                              <option value="">--------------</option>
                              <?PHP
                              ...
                              foreach ($arrKindMeetings as $key => $value) {
                                $kinId = $value['id'];
                                $kinDesc = $value['name'];?>
                              <option value="<?php echo $kinId?>" <?php if($item == $kinId){ echo "selected";} ?>><?php echo utf8_encode($kinDesc)?></option>
                              <?php } ?>
                            </select>
                          </div>
                          <label class="col-lg-2 text-right control-label font-bold">Posición</label>
                          <div class="col-lg-2">
                            <input type="text" class="form-control" placeholder="Posición" name="position" value="<?php if($position != ""){ echo $position; } ?>">
                          </div>
                          ...
                          <div class="col-lg-2">
                            <?php $result_check = 0;
                                  if($name_business !="" &&
                                     ...
                                     $email_contact !="" &&
                                     $phone_office !="" &&
                                     $cell_phone !=""){ $result_check = 1;} ?>
                            <a href="modal-verificado.php?id_suspect=<?php echo $suspectid?>&id_master_note=<?php echo $userrefid?>&campaign=<?php echo $_GET['id_campaign']; ?>&verify=<?php echo $result_check;?>" style="border-radius:100px; position:absolute;" title="Verificar Sospechoso" data-toggle="ajaxModal" class="btn btn-sm btn-icon"><i style="font-size:50px; <?php if($verified != 0){ echo 'color:#5DADE2'; } ?>" class="fa fa-check-circle"></i></a>
                          </div>
                        </div>
                        <div class="form-group">
                          <label class="col-lg-2 text-right control-label font-bold">Email</label>
                          <div class="col-lg-2">
                            <input type="text" class="form-control" placeholder="Email" name="email" value="<?php if($email != ""){ echo $email; } ?>">
                          </div>
                          <label class="col-lg-2 text-right control-label font-bold">Email</label>
                          <div class="col-lg-2">
                            <input type="text" class="form-control" placeholder="Email" name="email_contact" value="<?php if($email_contact != ""){ echo $email_contact; } ?>">
                          </div>
                        </div>
                        <div class="form-group">
                          <label class="col-lg-2 text-right control-label font-bold">Telefono</label>
                          <div class="col-lg-2">
                            <input type="text" class="form-control" placeholder="Telefono" name="phone_ofice" value="<?php if($phone_office != ""){ echo $phone_office; } ?>">
                          </div>
                          <label class="col-lg-2 text-right control-label font-bold">Telefono</label>
                          <div class="col-lg-2">
                            <input type="text" class="form-control" placeholder="Telefono" name="phone" value="<?php if($cell_phone != ""){ echo $cell_phone; } ?>">
                          </div>
                        </div>
                        <div class="form-group">
                          <label class="col-lg-2 text-right control-label font-bold">Dominio</label>
                          <div class="col-lg-2">
                            <input type="text" class="form-control" placeholder="Dominio" name="domain" value="<?php if($domain != ""){ echo $domain; } ?>">
                          </div>
                        </div>
                      <footer class="panel-footer text-right bg-light lter">
                        <a href="list_susp.php?id_campaign=<?php echo $_GET['id_campaign']; ?>" class="btn btn-default btn-s-xs">Regresar</a>
                        <button type="submit" name="submitSuspect" class="btn btn-primary btn-s-xs">Guardar</button>
                      </footer>
                    </div>
                    </section>
                  </form>
            ...
            <section class="panel panel-default">
              <header class="panel-heading">
                ...
                <?php $hora=date("H:i",strtotime($date_next_step)); ?>
                <span><a href="modal-next-step-suspect.php?next_step=<?php echo $suspectid?>&next_step_id=<?php echo $next_step?>&fecha=<?php echo $fecha?>&id_master_note=<?php echo $userrefid; ?>&hora=<?php echo $hora; ?>" title="Proximo Paso" data-toggle="ajaxModal" class="btn btn-sm btn-success">Próximo Paso</a></span>
                <?php $result_check = 0;
                      if($name_business !="" &&
                         $contacto !="" &&
                         $item !="" &&
